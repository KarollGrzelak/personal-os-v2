...
- `src/learning.js` contains the Learning domain: Roadmap, LessonGuide, validation, escaped Product UI v1 current-lesson and expandable-roadmap rendering, reconciliation, and LearningModule registration;
...
- `src/english.js` contains EnglishModule: strict profile and activity contracts, the manual queue and state machine, Task integration, and its escaped Product UI v1 current-activity, queue, creation, profile, and history view;
...
The English declarations in `src/english.js` depend on Core persistence, registration, calendar validation, and planning-date assertion, plus Learning's escaping and URL validation. Loading the layer defines strict validators and operations, then registers `EnglishModule`; it does not read or write Store, touch DOM, or emit events. The renderer orders the current activity and its single primary action, queue, labelled creation form, profile, and collapsed history; long queue and history projections use native disclosures. Internal status and priority values remain outside the normal presentation. English refreshes its own view after UI actions, restores focus to a deterministic surviving control, and relies on shared task events for Today. It exposes at most one current open Task at integer priority `39`, plus activities completed on the explicit planning date so completion can be undone even when the profile is disabled or invalid.
...
The IT learning module manages roadmap stage statuses, criterion progress, and LessonGuide content. `getTasks(date)` validates the explicit date while its current criterion pool remains date-independent; tasks use priority `40` and `planningClass: 'flexible'`. LessonGuide is attached content, never a task. The product view derives the next unfinished criterion without changing that contract and makes its topic, purpose, material, instructions, exercise, completion rule, and next action immediately visible. Missing guide fields stay explicit and neutral. The full roadmap follows as native stage disclosures; opening a disclosure is view-only. Imported or edited guide data uses domain validation and escaped rendering, raw JSON is advanced-only, and resource URLs are revalidated at render time and accept only HTTP or HTTPS.
...
The English module stores a strict editable profile and an ordered queue of atomic manual activities. At most one `todo` activity can be marked current. `getTasks(date)` uses only the explicit date for completed activities; exposed tasks use priority `39` and `planningClass: 'flexible'`. Status changes follow explicit `todo`, `done`, and `skipped` transitions; editing cannot alter status fields, and deleting the current activity never promotes another item automatically. The product view translates those states into user-facing wording, keeps long collections behind native disclosures, and moves focus after successful mutations. The weekly minute value remains informational, and the module does not generate lessons, contact a network service, or assess resource quality.
...
- User-controlled values in the PlanDay-powered Today view are inserted with `textContent` or text nodes. Product Training, School, Learning, and English views escape profile, task, note, lesson, guide, resource, identifier, and history values before HTML interpolation.
...
3. module contracts, compatibility decisions, Day/Habits, Training, Learning/LessonGuide, School, Availability, English, pure PlanDay algorithms, production Today integration, and Product UI Today, Training, School, Learning, and English states, hierarchy, disclosures, labels, focus, and interactions;
...
- Product UI v1 is implemented through Step 11.4B4 and awaits independent audit; settings and final cross-view accessibility remain bounded later steps.
